    ...
    while (OL_TRUE)
    {
        ...
        if (recv_comm.val_command != OL_COMMAND_NULL)
        {
            ...
            if (recv_comm.data_transferred_amt >= recv_comm.val_data.length)
            {
                ...
                {
                case OL_COMMAND_GET_RELAYS:
                    ol_command_get_relays_handler(send_comm_blox);
                    break;
                case OL_COMMAND_GET_CSV_ACTIVE:
                    ol_command_get_csv_active_handler(send_comm_blox);
                    break;
                case OL_COMMAND_GET_CSV_PROG:
                    ol_command_get_csv_prog_handler(send_comm_blox);
                    break;
                case OL_COMMAND_GET_CSV_CUR_STAT:
                    ol_command_get_csv_cur_stat_handler(send_comm_blox);
                    break;
                case OL_COMMAND_ALTER:
                    ol_command_alter_handler(send_comm_blox);
                    break;
                case OL_COMMAND_CSV_START:
                    ol_command_csv_start_handler(send_comm_blox);
                    break;
                case OL_COMMAND_CSV_PAUSE:
                    ol_command_csv_pause_handler(send_comm_blox);
                    break;
                case OL_COMMAND_CSV_STOP:
                    ol_command_csv_stop_handler(send_comm_blox);
                    break;
                case OL_COMMAND_ECHO:
                    ol_command_echo_handler(send_comm_blox);
                    break;
                default:
                    break;
                }
